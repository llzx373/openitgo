...
> 状态：已实施（一期/二期/三期 + 后续 TC 对齐迭代阶段 A–N + 二期增强阶段 O–AI）。
> 本文档保留需求与设计原貌；
...
## 11. 后续迭代（TC 对齐）实现记录（阶段 A–N + 二期 O–AI）
...
一期 A–N 之后，按差距清单剩余条目与用户反馈又完成了 21 个增强阶段（O–AI，
commit `2d9a60d`–`2fe603f`），全部已实施：

| 阶段 | 内容 | 对应差距清单条目 |
|------|------|------------------|
| O | 行为设置包：`fm_confirm_delete`/`fm_show_hidden`/`fm_dirs_first` 经 ui() 每帧下发，设置页「文件管理器」tab | §4：目录置顶/隐藏文件开关 |
| P | 键位补齐：F4 编辑器（未配置回退「打开方式…」）、Ctrl+D 目录热表、Ctrl+L 计算占用、Alt+F5 压缩/Alt+F9 解压、Shift+F5 副本、Ctrl+Shift+Enter 路径入命令行、双击空白 = 上级 | §10 键位表 |
| Q | 状态栏增强：当前盘剩余空间 + 过滤生效指示 | §三：状态栏密度 |
| R | 预览器增强：三模式（自动/文本/hex）、编码切换、内容搜索、图片旋转 | §7：F3 查看器 |
| S | 系统真实图标（Windows SHGetFileInfo + 缓存，跨平台回退语义图标） | §三：系统文件图标 |
| T | 过滤增强：通配 `*`/`?`、过滤方案保存（Ctrl+S，`fm_saved_filters`）、底栏过滤输入框 | §9.2 快速过滤 |
| U | 空白拖拽框选（rubber band，Ctrl 追加）+ 选择集保存/恢复（Num/） | §3.5/§3.6 |
| V | 三态视图 + 自定义列（`fm_columns`）+ Unsorted/Attr 排序 | §2.1/§2.2 |
| W | descript.ion 注释：注释列、Ctrl+Z 编辑、复制/移动/重命名/删除同步迁移 | §2.5 |
| X | 命令行条（面板下方直接执行命令，当前目录为工作目录） | §6.12 |
| Y | 自定义按钮栏（命令/目录/分隔符，`fm_button_bar` 持久化） | §6.13 |
| Z | 拖放/剪贴板/watch 收尾：`fm_drag_confirm` 拖放确认可关；剪贴板复制/剪切/粘贴（macOS NSPasteboard 待验证） | §四 12；§1.4 |
| AA | 任务队列（多操作排队、底栏任务面板、暂停/取消）+ 完成后错误汇总对话框 | §5.1/§5.10；§二 14 |
| AB | 复制校验（逐字节）/ 复制过滤 / 压缩暂停继续 | §5.2/§5.4 |
| AC | 校验和（CRC32/SHA-1/SHA-256 生成与验证，MD5 标不支持）+ 属性/时间戳对话框 | §6.7 |
| AD | 文件分割/合并（.001/.002… + .crc） | §6.5 |
| AE | 同步目录完整版（方向/递归对比/分类预览/生成任务组入队列） | §6.3 |
| AF | 按内容比较（二进制快比 + 文本 LCS 行级 diff 对照） | §6.4 |
| AG | 操作撤销（复制/移动/重命名/新建可逆；删除 = 回收站天然可恢复） | §二 13 |
| AH | 树形面板（Alt+F10，每栏独立懒展开树，宽度可调） | §1.9 |
| AI | 标签增强：标签组另存/载入（tc.tabs 文本格式）、`fm_tab_groups` 会话恢复扩展 | §1.1 |

原设计文档正文（§1–§10）保留需求与设计原貌不再逐处修订；与现状的出入以本节、
§10「实施偏差记录」及 AGENTS.md 为准。当前完整键位与设置项清单见 AGENTS.md
「文件管理器（app 侧）」段；差距清单各条目的实现状态标注见
`docs/fm-vs-totalcommander-gaps.md`（已全量更新为二期收尾后现状，含实施记录
与已知问题）。
