...
- [ ] `cargo fmt --all`、`cargo check --workspace`、`cargo test --workspace`、`cargo clippy --workspace --all-targets -- -D warnings` 全部通过。
- [ ] 书架以卡片/网格形式展示漫画封面缩略图。
- [ ] Webtoon 模式支持连续垂直滚动，滚轮不再整页翻页。
- [ ] 设置里的默认适配模式在阅读器打开时生效，缩放/平移交互增强。
- [ ] PDF/RAR 大文件翻页时不再重复全量读取。
- [ ] 设计文档与工程文件（LICENSE、AGENTS.md、CI）补齐。

---

**下一步建议**：从 P0 开始实施，先补齐书架缩略图和 Webtoon 连续滚动，这两项对用户体验提升最直接。
