> **Status:** 已归档。本文档记录了 2026-06-17 的设计快照，后续已多次重构；当前代码在加载器（多 IO 线程 + 多解码队列）、纹理后端（wgpu + DXT5 解压）、书架（封面补生成、已删除检测、菜单栏）等方面与本文档存在差异。请以根目录 `README.md`、`CHANGELOG.md` 为准。
>
> **注意：** 文档中的 TODO 编号为历史编号，详见 `TODO.md` 中的「历史 TODO 编号对照表」。
...
2. **阅读器视图（Reader）**
   - 中央：漫画渲染区（单页或双页跨页）。
   - 顶部工具栏：返回书架、模式切换、单/双页切换、缩放、页码跳转、添加书签、全屏、设置；右上角有 ✕ 按钮可隐藏。
   - 底部进度条：持续显示当前阅读进度，点击可跳转到对应页；右上角有 ✕ 按钮可隐藏。
   - 缩略图预览：鼠标悬停在底部进度条上时，在鼠标位置上方显示对应页的缩略图（已缓存页显示纹理，未缓存页显示页码占位），点击进度条可跳转。
   - 右键菜单：下一页、上一页、首页、末页、添加书签、全屏、返回书架、显示/隐藏工具栏、显示/隐藏进度条。

3. **设置视图（Settings）**
   - 通用：主题、默认阅读模式、窗口状态。
   - 阅读：缓存大小（100 MB - 4 GB，默认 1 GB）、反转滚轮方向（适配 macOS 自然滚动）。
...
| +/- / 0 | 缩放 / 自动适应 |
| 拖拽 | 放大时平移 |
| 右键 | 页面上下文菜单（含工具栏/进度条显示切换） |
| 双击 | 切换全屏 |
| 鼠标悬停底部进度条 | 在光标上方显示对应页缩略图预览 |
...
- [x] **可隐藏工具栏/进度条**：顶部工具栏和底部进度条可分别隐藏/显示，状态持久化。
- [x] **底部进度条 + 悬停缩略图**：底部持续显示阅读进度条；鼠标悬停时根据光标位置显示对应页的缩略图预览，点击进度条跳转。
