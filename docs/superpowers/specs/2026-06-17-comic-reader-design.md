...
2. **阅读器视图（Reader）**
   ...
   - 底部状态栏：页码、阅读模式、缩放比例、单/双页状态、快捷键提示；右上角有 ✕ 按钮可隐藏。
   - 悬浮缩略图进度条：鼠标移到阅读区底部 24px 范围内时滑出，显示当前卷所有页面缩略图（已缓存页显示纹理，未缓存页显示页码占位），点击跳转。
...
| +/- / 0 | 缩放 / 自动适应 |
| 拖拽 | 放大时平移 |
| 右键 | 页面上下文菜单（含工具栏/状态栏显示切换） |
| 双击 | 切换全屏 |
| 鼠标移到阅读区底部 | 显示悬浮缩略图进度条 |
...
- [x] **可隐藏工具栏/状态栏**：顶部工具栏和底部状态栏可分别隐藏/显示，状态持久化。
- [x] **悬浮缩略图进度条**：鼠标悬停阅读区底部时显示缩略图进度条，点击跳转。
