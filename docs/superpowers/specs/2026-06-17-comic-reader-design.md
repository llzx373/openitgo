...
2. **阅读器视图（Reader）**
   ...
   - 底部进度条：持续显示当前卷所有页面的页码导航，点击页码跳转；右上角有 ✕ 按钮可隐藏。
   - 悬浮缩略图预览：鼠标悬停在底部进度条上时，在进度条上方弹出缩略图条，显示当前卷所有页面缩略图（已缓存页显示纹理，未缓存页显示页码占位），点击跳转。
...
- [x] **底部进度条 + 缩略图预览**：底部持续显示页码进度条；鼠标悬停进度条时弹出缩略图预览，点击跳转。
