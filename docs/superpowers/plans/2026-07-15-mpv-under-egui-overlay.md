...
**Architecture:** eframe 0.29 原生支持透明 backbuffer：`ViewportBuilder::with_transparent(true)` 使 egui-wgpu 选择 `CompositeAlphaMode::PreMultiplied`，wgpu-hal 随之把 CAMetalLayer 设为 `opaque=NO`；`App::clear_color` 返回全透明后，egui 未绘制的区域透出下层内容。winit view 的 `layer` 本身就是 wgpu 的 CAMetalLayer（wgpu-hal 将其复用为 view 主 layer——Task 1 运行时层树 dump 实证，见文末修订记录）；把 mpv 的 CAOpenGLLayer 插入该 CAMetalLayer 的 superlayer 中、以 `insertSublayer:below:` 锚定在 metal 层之下，视频即在 egui 之下合成。鼠标事件仍由 egui 的 NSView 接收（hit-test 按几何不按 alpha），交互路径不变。
...
> **执行记录（2026-07-15）**：按原文执行时前提被证伪（轮 1：`view.layer` 即 CAMetalLayer 本体，`insertSublayer:atIndex:0` 把层插进 metal 层内部，遮盖全部 egui 内容，验收第 2 条失败触发停止条件）；改用修正机制"metal 层 superlayer 中 `insertSublayer:below:` 锚定"后验收 5/5 通过（轮 2）。报告/证据：`.superpowers/sdd/task-1-report{,-round2}.md`、`task-1-evidence{,/round2}/`（未入库）。下方原文保留作为实验设计记录；Task 4 已按修正机制修订（见文末修订记录）。
...
关键背景（实现前必读）：
- **插入机制（Task 1 轮 2 实证）**：winit view 的 `layer` **本身就是** wgpu 的 CAMetalLayer（wgpu-hal 复用为 view 主 layer；原计划"metal 层是主 layer 的 sublayers[0]"被运行时层树 dump 证伪）。视频层必须插到该 metal 层的 **superlayer** 中、用 `insertSublayer:below:` 锚定在 metal 层之下——CA 规则下 sublayer 永远画在父层 content 之上，**插进 metal 层内部必然遮盖一切 egui 内容**（轮 1 实测）。插入前做 `isKindOfClass:CAMetalLayer` 检查并打日志（wgpu/winit 升级改变层结构时第一时间发现）；`superlayer == nil`（view 未挂进窗口层树）时返回错误、由上层重试，**不要** fallback 到插进 metal 层内部。probe 窗口（无 wgpu，`setWantsLayer` 后 view.layer 是普通 CALayer）没有 CAMetalLayer：`isKindOfClass` 为假时退回 `insertSublayer:atIndex:0`，保证 `probe_visible` 等裸窗口 probe 继续可用。
- **坐标系（Task 1 轮 2 实证）**：该层栈的 superlayer 为左上角原点（winit 设了 geometryFlipped——红层 (0,60,900,520) 精确落在距顶 60pt 处）。`make_frame` 直通转换预期正确；接线后先用一个非对称矩形实测一次再信，若上下错位仍按下面的 y 翻转备选公式处理并记录实测结果。
- **层树稳定性（watch 项）**：viewport resize（surface reconfigure）不影响兄弟层序（Task 1 轮 2 resize 后 dump 逐行一致）；fullscreen 切换 probe 未覆盖，AppKit 可能重建窗口层树。本任务不做预防性重锚定，但 Step 6 验收必须包含进出 fullscreen 检查；若失败，在 struct 保存 `ns_view`/`metal_layer` 原始指针（不 retain——winit view/window 寿命长于任何单次播放），`set_bounds` 里比对 `[ns_view layer]` 身份、变化时重跑 below 插入（fullscreen 必触发窗口尺寸变化 → 必然走到 `set_bounds`）。
...
```rust
        ...
        let (layer, osd_layer) = unsafe {
            // The winit view's layer IS wgpu's CAMetalLayer (wgpu-hal adopts
            // it as the view's main layer — Task 1 layer-dump verified), so
            // "index 0 of the view layer" would put the video INSIDE the
            // metal layer, where sublayers always composite above the
            // parent's content (Task 1 round 1 failure). Anchor the video
            // layer BELOW the metal layer in its superlayer instead. Probe
            // windows without wgpu have a plain CALayer after setWantsLayer:
            // fall back to index 0 there. These checks must run before any
            // layer/state allocation so the Err paths stay leak-free.
            let () = msg_send![ns_view, setWantsLayer: YES];
            let metal_layer: *mut Object = msg_send![ns_view, layer];
            let is_metal: bool =
                msg_send![metal_layer, isKindOfClass: class!(CAMetalLayer)];
            let parent_layer: *mut Object = if is_metal {
                let parent: *mut Object = msg_send![metal_layer, superlayer];
                if parent.is_null() {
                    return Err(
...
Expected: 全绿（mpv_view 的 make_frame/osd_frame 测试不受影响）。probe_visible 正常出画面（裸层走 `atIndex:0` fallback，在 probe 的裸窗口 layer 树下同样工作）。真机打开视频：画面位置正确、无上下颠倒、OSD 右上角可见并淡出；菜单栏菜单与字幕/音轨/输出下拉框**悬浮在视频之上**；滚轮音量/快捷键正常（事件仍由 egui 接收）。**进出 fullscreen：视频位置/尺寸保持正确，菜单/弹层混色仍在视频之上**（若层树被 AppKit 重建导致失败，按关键背景"层树稳定性 watch 项"补重锚定逻辑）。若视频上下错位，按任务开头背景说明应用 y 翻转公式。
...
- **2026-07-15（Task 1 执行后）**：原计划前提"wgpu 的 CAMetalLayer 是 winit view 主 layer 的 sublayers[0]"被运行时层树 dump 证伪——`view.layer` 本身就是 CAMetalLayer（wgpu-hal 复用为 view 主 layer），`insertSublayer:atIndex:0` 实际插进 metal 层内部，红层遮盖全部 egui 内容（Task 1 轮 1，验收第 2 条失败触发停止条件）。修正机制"metal 层 superlayer 中 `insertSublayer:below:` 锚定"经 Task 1 轮 2 验收 5/5 通过（决定性判别项：egui 弹层与红层混色正常、resize 后 frame 跟随且 z 序不变、窗口阴影正常）。已同步修订：Architecture 段、Task 4 关键背景/Step 1/Step 3/Step 6、Task 6 的 AGENTS.md 新文本；Task 1 原文保留作为实验设计记录。证据与报告：`.superpowers/sdd/task-1-evidence{,/round2}/`、`task-1-report{,-round2}.md`（未入库）；probe 代码见 `rust-reader-app/examples/probe_overlay.rs`。
