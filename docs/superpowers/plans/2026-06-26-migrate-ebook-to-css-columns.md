...
> 状态：Phase 5 部分完成（前 3 项已落地，大章节内存优化列为未来工作）  
...
- [x] 缓存页数/布局结果：JS 侧引入 `layoutCache`，以章节 HTML 哈希 + 视口尺寸 + 边距 + 字号 + 行高 + 模式 + 栏间距为 key；`layout()` 命中缓存时直接恢复 `totalPages`/`pageWidth`/`viewShift`。设置变化或切换章节时显式失效缓存。
- [x] 减少 resize 时的重新布局：resize 处理在防抖后比较当前与上次布局的宽度/高度/模式；宽度和模式未变时直接跳过；滚动模式下仅高度变化无需处理；分页模式下仅高度变化时调用 `recomputeTotalPages()` 重算总页数并保留当前 spread。
- [x] 相邻章节轻量预加载：新增 `preloadChapter(index)`，异步获取章节 HTML 并解析到 inert `<template>`，不进入可见 column 容器；`loadChapter()` 完成后自动预加载前后章；Rust 侧暴露 `preload_chapter` 并在 `goto_chapter` 后调用。
- [ ] 针对大章节优化内存（考虑分段加载 / 虚拟化）：当前整章 HTML 仍一次性注入 DOM，超长章节可能占用较多内存。此项列为未来优化，不在本阶段实现。
...
- 布局缓存已落地：相同章节 + 相同视口/设置组合不再触发完整 CSS columns 回流。
- resize 处理已优化：宽度/模式未变时跳过布局；滚动模式高度变化由浏览器处理；分页模式高度变化仅重算总页数。
- 相邻章节预加载已落地：JS 自动预加载、Rust 暴露 `preload_chapter`，均通过 inert `<template>` 实现，不影响可见布局。
- 大章节内存优化尚未实现，保留为未来工作项。

## 11. 下一步行动

1. 继续观察 Phase 5 优化在真实 EPUB 上的效果（布局缓存命中率、resize 响应延迟、预加载收益）。
2. 评估大章节内存优化方案（如分段加载、虚拟化或限制单章最大字数），在后续迭代中实现。
