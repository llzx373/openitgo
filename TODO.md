...
- [x] 57. 依赖升级评估：egui 0.29 → 上游 0.33+（牵动 wgpu 与既有 API workaround）；跟踪 `pdf-render` beta 迭代；合并双 PDF 栈（parser `pdf 0.9` 取页数 + app `pdf-render` 渲染）（PDF 栈已合并）；`objc 0.2` → objc2（objc2 迁移已完成：openitgo-app 直接依赖改为 objc2 0.6 + objc2-core-foundation 0.3，aarch64-only 限制已解除，x86_64 交叉 check 通过）（egui 已升级 0.29 → 0.35.0：eframe `App` trait `update` → `ui`、`TopBottomPanel`/`SidePanel` 统一为 `Panel`、`menu::bar` → `MenuBar`、`Rounding` → `CornerRadius`、`hardware_acceleration` 字段移除（wgpu 默认 HighPerformance 等价）、图标库 egui-phosphor → egui_phosphor_icons 0.4；egui 0.35 要求 rustc 1.92+，本机 stable 已升 1.97.1；`pdf-render` 上游仍为 beta（1.0.0-beta.17），保持 beta.9 跟踪）
