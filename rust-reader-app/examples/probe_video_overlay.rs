...
//! Usage: cargo run -p rust-reader-app --example probe_video_overlay -- <video-file>
...
mod imp {
    ...
    impl ProbeApp {
        ...
        /// (RustReaderMpvLayer) and the CAMetalLayer so their relative order
        /// is explicit.
        ///
        /// # Safety
        ...
        unsafe fn dump_layer_tree(view_layer: *mut Object) {
            unsafe {
                ...
                for (i, s) in ssubs.iter().enumerate() {
                    ...
                    if name == "RustReaderMpvLayer" {
                        video_idx = Some(i);
                    }
                    if *s == view_layer {
                        metal_idx = Some(i);
                    }
                    let tag = if name == "RustReaderMpvLayer" {
                        "  ← VIDEO (mpv)"
    ...
    pub fn run(path: String) {
        ...
        eframe::run_native(
            "RUSTREADER_PROBE_VIDEO_OVERLAY",
            options,
            Box::new(|_cc| {
                Ok(Box::new(ProbeApp {
                    player: None,
